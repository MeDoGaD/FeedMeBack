...
class QuickSort {
  ...
  // quicksort the array
  static void sort(List<Comment> a) {
    _shuffle(a);
    _sortWithComparable(a, 0, a.length - 1);
  }

  // quicksort the subarray from a[lo] to a[hi]
  static void _sortWithComparable(List<Comment> a, int lo, int hi) {
    if (hi <= lo) {
      return;
    }
  ...
  static int _partition(List<Comment> a, int lo, int hi) {
    int i = lo;
    int j = hi + 1;
    Comparable v = a[lo].date;

    while (true) {

      // find item on lo to swap
      while (_less(a[++i].date, v)) {
        if (i==hi) {
          break;
        }
      }
  ...
  static void _shuffle(List a) {
    try {
      ...
      for (int i = 0; i < N; i++) {
        var rndm = new Random();
        int r = i + (rndm.nextInt(100) * (N - i)).toInt();
        String swap = a[r];
        a[r] = a[i];
        a[i] = swap;
      }
   ...
   ***********************************************************************/
  static Comment select(List<Comment> a, int k) {
    if (k < 0 || k >= a.length) {
  ...
  // is the list sorted
  static bool _isSorted(List<Comment> a) {
    return _isSortedInRange(a, 0, a.length-1);
  }

  // is the list sorted from a[lo] to a[hi]
  static bool _isSortedInRange(List<Comment> a, int lo, int hi) {
    for (int i= lo + 1; i <= hi; i++) {
      if (_less(a[i].date, a[i-1].date)) {
        return false;
      }
    }
